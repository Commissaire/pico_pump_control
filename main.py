...
while True:
    ...
    # Check if the sensor is triggered and the pump start time has been recorded
        ...
        if (utime.time() - pump_start_time_seconds < 3) \
                and global_pump_runtime_after_water_level_reset < MAX_GLOBAL_PUMP_RUNTIME:
            # Turn the pump on
            pump.value(1)
            pump_on = True
        # Check if the maximum global pump runtime has been reached
    ...
    # If sensor is triggered and the pump stopped more than 3 seconds ago,
    # reset pump start time and stop time to 0 so the pump can be turned on again
    if (sensor_1.value() == 0 or sensor_2.value() == 0) and pump_stop_time_seconds != 0:
        if (
                utime.time() - pump_stop_time_seconds > 3) \
                and global_pump_runtime_after_water_level_reset < MAX_GLOBAL_PUMP_RUNTIME:
            pump_start_time_seconds = 0
            pump_stop_time_seconds = 0

            # Calculate the global pump runtime after the water level reset button is pressed
    if pump_on is True and last_pump_status == 0:
        pump_on_time_start_ms = utime.ticks_ms()
        last_pump_status = 1
    elif pump_on is False and last_pump_status == 1:
